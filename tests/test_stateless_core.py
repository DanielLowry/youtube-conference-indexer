...
def test_run_extraction_search_multiple_queries_dedupes_across_inputs(monkeypatch, tmp_path: Path):
    """Search mode should accept multiple queries and emit one deduped output set."""
    search_calls = []

    def fake_search_list(**kwargs):
        query = kwargs["query"]
        search_calls.append((query, kwargs.get("page_token")))
        if query == "allocator":
            return {
                "items": [
                    {"id": {"videoId": "VID1"}},
    ...
    assert result.status == RunStatus.SUCCEEDED
    assert result.progress.pages_processed == 2
    assert result.progress.total_queries == 2
    assert result.progress.processed_queries == 2
    assert result.progress.results_seen == 4
    assert result.progress.existing_video_ids == 1
    assert result.progress.new_video_ids == 3
    assert result.progress.videos_fetched == 3
    assert search_calls == [("allocator", None), ("pmr", None)]
    ...
    assert len(rows) == 3
    assert {row["source_query"] for row in rows} == {"allocator", "pmr"}


def test_search_max_pages_applies_per_query(monkeypatch, tmp_path: Path):
    """Max pages should cap each query independently, not the entire run."""
    search_calls: list[tuple[str, str | None]] = []

    def fake_search_list(**kwargs):
        query = kwargs["query"]
        page_token = kwargs.get("page_token")
        search_calls.append((query, page_token))
        if query == "allocator" and page_token is None:
            return {"items": [{"id": {"videoId": "VID1"}}], "nextPageToken": "allocator-page-2"}
        if query == "pmr" and page_token is None:
            return {"items": [{"id": {"videoId": "VID2"}}], "nextPageToken": "pmr-page-2"}
        raise AssertionError("Extractor should not request second pages when max_pages=1")
...
def test_run_extraction_playlist_mode_supports_multiple_playlists(monkeypatch, tmp_path: Path):
    """Playlist mode should scan multiple playlist IDs into one deduped run output."""
...
def test_playlist_max_pages_applies_per_playlist(monkeypatch, tmp_path: Path):
    """Max pages should cap each playlist independently, not the entire run."""
    playlist_calls: list[tuple[str, str | None]] = []

    def fake_playlist_items_list(playlist_id: str, page_token: str | None = None, max_results: int = 50):
        playlist_calls.append((playlist_id, page_token))
        assert max_results == 50
        if playlist_id == "PLA" and page_token is None:
            return {
                "items": [{"contentDetails": {"videoId": "VID1"}}],
                "nextPageToken": "pla-page-2",
            }
        if playlist_id == "PLB" and page_token is None:
            return {
                "items": [{"contentDetails": {"videoId": "VID2"}}],
                "nextPageToken": "plb-page-2",
            }
        raise AssertionError("Extractor should not request second pages when max_pages=1")
    ...
    assert result.progress.pages_processed == 2
    assert result.progress.processed_playlists == 2
    assert playlist_calls == [("PLA", None), ("PLB", None)]
...
def test_channel_max_pages_applies_per_discovered_playlist(monkeypatch, tmp_path: Path):
    """Channel mode should apply max pages to each discovered playlist."""
    ...
    def fake_playlist_items_list(playlist_id: str, page_token: str | None = None, max_results: int = 50):
        ...
        if playlist_id == "PLB" and page_token is None:
            return {
                "items": [{"contentDetails": {"videoId": "B1"}}],
                "nextPageToken": "plb-page-2",
            }
        raise AssertionError("Extractor should not request second pages when max_pages=1")
